...
//  splayTree
//
//  Created by Isabel Vaca on 02/11/25.
//
#ifndef SplayTree_H_
#define SplayTree_H_
...
Node<T>* Node<T>::add(T val) {
    if (val < value) {//izq
        if (left != 0) {//verificar que exista
            return left->add(val);
        }
        else {
            left = new Node<T>(val);//se crea
            left->parent = this;//enlazar con padre (actual)
            return left;
        }
    } else if (val > value) {//der
        if (right != 0) {
...
Node<T>* Node<T>::remove(T val) {
    ...
    if (val < value) {//valor en la rama izq
        if (left == 0) {//si no tiene hijo derecho
            return this;
        }
        else if (left->value == val) {
            old = left;//valor a borrar
            if (old->right != 0) {  //si tiene hijo derecho
                succ = old->succesor();
                succ->left  = old->left;//amarrar hijos
                if (succ->left!=0){
                    succ->left->parent  = succ;//conectar con el padre
                }
            ...
            return left->remove(val);//seguir buscando
        }
...
Node<T>* Node<T>::find(T val){
    if (value == val) {
        ...
        return left  ? left->find(val)  : this; //checa si hay left 
    } else { // val > value
